...
public class Percolation {
	...
	public void open(int row, int column) {
		...
		if (!sites[siteIndex]) {
			...
			if (row == 0) {
				// If the requested site is a top site, connect it with virtual
				// top site
				weightedQUF.union(siteIndex, virtualTop);
			} else if ((row == siteLength - 1) && isFull(row, column)) {
				// If the requested site is a bottom site & if it's already a
				// full site, connect it with virtual bottom site
	...
	private void connectWithLeftNeighbor(int row, int column) {
		// Check that given site is not the first column and left neighbor is
		// open already
	...
	private void connectWithRightNeighbor(int row, int column) {
		// Check that given site is not the last column in the system and right
		// neighbor is open already
	...
	private void connectWithTopNeighbor(int row, int column) {
		// Check that given site is not the first row and top neighbor is open
		// already
